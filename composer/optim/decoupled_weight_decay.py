...
class DecoupledLionW(Optimizer):
    """Lion optimizer with the weight decay term decoupled from the learning rate.
    ...
    @staticmethod
    def lionw(params, grad, exp_avg, lr, initial_lr, weight_decay, beta1, beta2) -> None:
        r"""Functional API that performs LionW algorithm computation with decoupled weight decay.
        ...
        if weight_decay != 0:
            decay_factor = (lr / initial_lr) if initial_lr else 1.0
            params.data.mul_(1 - decay_factor * weight_decay)
    ...
    def step(self, closure: Optional[Callable] = None):
        ...
        for group in self.param_groups:
            for p in filter(lambda p: p.grad is not None and p.requires_grad, group['params']):
                ...
                grad, lr, initial_lr, wd, beta1, beta2, state = p.grad, group['lr'], group['initial_lr'], group[
